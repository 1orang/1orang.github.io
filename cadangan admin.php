...
$attitude = floatval($empData['attitude']);
           ...
           $grade = ($score >= 100) ? 'A' : (($score >= 80) ? 'B' : (($score >= 60) ? 'C' : 'D'));
...
<form method="POST" action="">
    ...
    <button class="btn save-btn" type="submit" name="save" onclick="return confirmSave()">Simpan</button>
...
// Optional: Konfirmasi sebelum simpan
function confirmSave() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            document.querySelector('form').submit(); // submit form manual
        }
    });
    return false; // cegah submit otomatis
}
</script>
